...
namespace Nop.Plugin.Payments.SIMAuthorizeNet
{
    ...
    public class PostPage
    {
        ...
        private NameValueCollection NameValues = new NameValueCollection();
        ...
        {
            if (url == string.Empty)
                throw new NotSupportedException("Must input the new Page.!!!");
            _url = url;
        }
        ...
        public void Post()
        {
            ...
            res.Clear();
            res.Write(string.Format("<html><head></head><body onload=\"document.forms['form1'].submit();\"><form name=\"form1\" id=\"form1\" action=\"{0}\" method=\"post\">", _url));
            for (int i = 0; i < NameValues.Keys.Count; i++)
            {
                string name = NameValues.Keys[i];
                string value = NameValues[NameValues.Keys[i]];
                if (NameValues.Keys[i] != "x_line_item")
                {
                    res.Write(string.Format("<input type=\"hidden\" id=\"{0}\" name=\"{0}\" value=\"{1}\">", NameValues.Keys[i], NameValues[NameValues.Keys[i]]));
                }
                else
                {
                    var arrItems = Utils.SplitByString(NameValues[NameValues.Keys[i]], HttpUtility.HtmlAttributeEncode("<^>"));
                    for (int j = 0; j < arrItems.Length; j++)
                    {
                        res.Write(string.Format("<input type=\"hidden\" id=\"{0}\" name=\"{0}\" value=\"{1}\">", "x_line_item", arrItems[j]));
                    }
                }
