...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            try
            {
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    if (!processPaymentRequest.IsRecurringPayment)
                    {
                        ...
                        foreach (var sc in details.Cart)
                        {
                            ...
                            var orderItem = new OrderItem
                            {
                                ...
                                FullRodWidth = sc.SelectedRodWidth > 0 ? (sc.SelectedRodWidth.ToString("G29") + (sc.SelectedRodWidtheven > 0 ? " " + CommonHelper.ConvertSizeToString((double)sc.SelectedRodWidtheven) : "") + "\"") : String.Empty
                            };
                    ...
                    //check if order is marked as paid or authorize
                    if (processPaymentResult.NewPaymentStatus == PaymentStatus.Authorized || processPaymentResult.NewPaymentStatus == PaymentStatus.Paid)
                    {
                        SendMainNotification(order);
        ...
        public void SendMainNotification(Order order)
        {
            //send email notifications
            ...
            var vendors = GetVendorsInOrder(order);
            var poTemps = new List<POTemp>();
            foreach (var vendor in vendors)
            {
                // po template by product type
                var poTemplates = _pOTemplateMessageService.GetAllPOTemplateMessages(vendor.Id).Where(x=>x.Active).ToList();
                foreach (var item in order.OrderItems)
                {
                    if (item.Product.VendorId == vendor.Id)
                    {
                        foreach (var template in poTemplates)
                        {
                            if (template.POTemplate_ProductTypes.Any(x => x.ProductTypeId == item.Product.ProductTypeId))
                            {
                                var poTemp = poTemps.FirstOrDefault(x => x.POTemplateMessage.Id == template.Id);
                                if (poTemp != null)
                                {
                                    poTemp.OrderItems.Add(item);

                                }
                                else
                                {
                                    var _newPoTemp = new POTemp()
                                    {
                                        OrderId = order.Id,
                                        POTemplateMessage = template
                                    };

                                    _newPoTemp.OrderItems.Add(item);
                                    poTemps.Add(_newPoTemp);
                                }
                                break;
                            }
                        }
                ...
                //create purchase orders and send email
                foreach (var _temp in poTemps)
                {
                    var po = new PurchaseOrder()
                    {
                        ...
                        OrderId = order.Id,
                        PONumber = "PO" + (_temp.OrderId + "" + vendor.Id + _temp.POTemplateMessage.Id).PadLeft(10, '0')
                    };
                    //create content and send email to supplier

                    int orderSupplierNotificationQueuedEmailId = _workflowMessageService.SendPOSupplierNotification(po, _temp.POTemplateMessage, _temp.OrderItems, order, vendor, order.CustomerLanguageId,_productService);
                    if (orderSupplierNotificationQueuedEmailId > 0)
                    {
                        order.OrderNotes.Add(new OrderNote
                        {
                            Note = string.Format("Purchase Order (Id=" + po.PONumber + ") email (to supplier) has been queued. Queued email identifier: {0}.", orderSupplierNotificationQueuedEmailId),
                            DisplayToCustomer = false,
                            CreatedOnUtc = DateTime.UtcNow
                        });
                        _orderService.UpdateOrder(order);
                        _purchaseOrderService.InsertPO(po);
                    }
                }
        ...
        private void SplitPOItem(OrderItem item, POTemp poTemp)
        {
            var _colorAttr = this._productAttributeParser.ParseProductAttributeMappings(item.AttributesXml)
                        .Where(x => x.AttributeControlType == AttributeControlType.ColorSquares
                            && x.ProductAttribute.Name.Trim() == "Color").FirstOrDefault();
            var _selectedColorId = this._productAttributeParser.ParseValues(item.AttributesXml, _colorAttr.Id).FirstOrDefault();
            var colorAttributeXml = this._productAttributeParser.AddProductAttribute(string.Empty, _colorAttr, _selectedColorId);
            //split product
            //finials
            if (item.Product.FinialProductId.HasValue)
            {
                var _finial = this._productService.GetProductById(item.Product.FinialProductId.Value);
                if (_finial != null)
                {
                    poTemp.OrderItems.Add(new OrderItem()
                    {
                        Quantity = item.Product.NumberOfFinials * item.Quantity,
                        ProductId = _finial.Id,
                        Product = new Product()
                        {
                            Id = _finial.Id,
                            Name = _finial.Name,
                            NameInPO = _finial.NameInPO,
                            Packaged = item.Product.PackagedFinials,
                            ProductTypeId = (int)ProductType.Finials
                        },
                        AttributesXml = colorAttributeXml
                    });

                }
            }
            //pole
            if (item.Product.PoleProductId.HasValue)
            {
                var _pole = this._productService.GetProductById(item.Product.PoleProductId.Value);
                if (_pole != null)
                {
                    poTemp.OrderItems.Add(new OrderItem()
                    {
                        Quantity = item.Product.NumberOfPoles * item.Quantity,
                        ProductId = _pole.Id,
                        Product = new Product()
                        {
                            Id = _pole.Id,
                            Name = _pole.Name,
                            NameInPO = _pole.NameInPO,
                            Packaged = item.Product.PackagedPoles,
                            ProductTypeId = (int)ProductType.Poles
                        },
                        AttributesXml = colorAttributeXml
                    });
            ...
            //bracket
            var _bracketsAttr = this._productAttributeParser.ParseProductAttributeMappings(item.AttributesXml)
                        .Where(x => x.AttributeControlType == AttributeControlType.RadioList
                            && x.ProductAttribute.Name.Trim() == "Brackets").FirstOrDefault();
            var _selectedBracketValue = this._productAttributeParser.ParseProductAttributeValues(item.AttributesXml).Where(x => x.ProductAttributeMappingId == _bracketsAttr.Id).FirstOrDefault();
            if (_bracketsAttr != null)
            {
                poTemp.OrderItems.Add(new OrderItem()
                {
                    Quantity = item.Product.NumberOfBrackets * item.Quantity,
                    ProductId = 0,

                    Product = new Product()
                    {
                        Id = 0,
                        Name = _selectedBracketValue.Name,
                        Packaged = item.Product.PackagedBrackets,
                        ProductTypeId = (int)ProductType.Brackets
                    },
                    AttributesXml = colorAttributeXml
                });
            }
            //ring
            var _ringQtyAttr = this._productAttributeParser.ParseProductAttributeMappings(item.AttributesXml)
                        .Where(x => x.AttributeControlType == AttributeControlType.RadioList
                            && x.ProductAttribute.Name.Trim() == "Ring Quantity").FirstOrDefault();
            if (_ringQtyAttr != null)
            {
                var _selectedRingQtyValue = this._productAttributeParser.ParseProductAttributeValues(item.AttributesXml)
                    .Where(x => x.ProductAttributeMappingId == _ringQtyAttr.Id).FirstOrDefault();
                if (_selectedRingQtyValue != null)
                {
                    poTemp.OrderItems.Add(new OrderItem()
                    {
                        Quantity = _selectedRingQtyValue.Quantity * item.Quantity,
                        ProductId = 0,

                        Product = new Product()
                        {
                            Id = 0,
                            Name = _selectedRingQtyValue.Name,
                            NameInPO = _selectedRingQtyValue.SupplierAttributeCode,
                            Packaged = _selectedRingQtyValue.Packaged,
                            ProductTypeId = (int)ProductType.Rings
                        },
                        AttributesXml = colorAttributeXml
                    });
                }
        ...
        public class POTemp
        {
            ...
            public POTemplateMessage POTemplateMessage { get; set; }
