...
namespace Nop.Services.Shipping
{
    ...
    public partial class ShippingService : IShippingService
    {
        ...
        private readonly IRepository<ShippingFeeByOrderTotal> _shipmentFeeByOrderTotalRepository;
        ...
        public ShippingService(IRepository<ShippingMethod> shippingMethodRepository,
            ...
            IRepository<ShippingFeeByOrderTotal> shipmentFeeByOrderTotalRepository,
            ILogger logger,
            ...
            ICacheManager cacheManager)
        {
            ...
            this._shipmentFeeByOrderTotalRepository = shipmentFeeByOrderTotalRepository;
        ...
        public IList<ShippingFeeByOrderTotal> GetAllShippingFees()
        {
            var query = _shipmentFeeByOrderTotalRepository.Table;

            query = query.OrderBy(s => s.MinValue);

            var result = query.ToList();
            return result;
        }

        public ShippingFeeByOrderTotal GetShippingFeeById(int Id)
        {
            if (Id == 0)
                return null;

            return _shipmentFeeByOrderTotalRepository.GetById(Id);
        }

        public void InsertShippingFee(ShippingFeeByOrderTotal shipping)
        {
            if (shipping == null)
                throw new ArgumentNullException("shipmentFee");

            _shipmentFeeByOrderTotalRepository.Insert(shipping);

            //event notification
            _eventPublisher.EntityInserted(shipping);
        }

        public void UpdateShippingFee(ShippingFeeByOrderTotal shipping)
        {
            if (shipping == null)
                throw new ArgumentNullException("shipmentFee");

            _shipmentFeeByOrderTotalRepository.Update(shipping);

            //event notification
            _eventPublisher.EntityUpdated(shipping);
        }

        public void DeleteShipmentFee(ShippingFeeByOrderTotal shipping)
        {
            if (shipping == null)
                throw new ArgumentNullException("shipmentFee");

            _shipmentFeeByOrderTotalRepository.Delete(shipping);

            //event notification
            _eventPublisher.EntityDeleted(shipping);
        }

        public decimal GetShippingFeeByTotalOrder(decimal orderTotal)
        {
            var query = _shipmentFeeByOrderTotalRepository.Table;
            var result = query.Where(s => s.MinValue <= orderTotal && s.MaxValue >= orderTotal).FirstOrDefault();
