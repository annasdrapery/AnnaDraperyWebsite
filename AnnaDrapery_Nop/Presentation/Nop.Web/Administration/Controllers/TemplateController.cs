...
namespace Nop.Admin.Controllers
{
    public partial class TemplateController : BaseAdminController
    {
        ...
        [HttpPost]
        public ActionResult BrandTemplates(DataSourceRequest command)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageMaintenance))
                return AccessDeniedView();

            var templatesModel = _brandTemplateService.GetAllBrandTemplates()
                .Select(x => x.ToModel())
                .ToList();
            var gridModel = new DataSourceResult
            {
        ...
        [HttpPost]
        public ActionResult BrandTemplateUpdate(BrandTemplateModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageMaintenance))
                return AccessDeniedView();
            ...
            var template = _brandTemplateService.GetBrandTemplateById(model.Id);
            if (template == null)
                throw new ArgumentException("No template found with the specified id");
            template = model.ToEntity(template);
            _brandTemplateService.UpdateBrandTemplate(template);

            return new NullJsonResult();
        }

        [HttpPost]
        public ActionResult BrandTemplateAdd([Bind(Exclude = "Id")] BrandTemplateModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageMaintenance))
                return AccessDeniedView();
            ...
            template = model.ToEntity(template);
            _brandTemplateService.InsertBrandTemplate(template);

            return new NullJsonResult();
        }

        [HttpPost]
        public ActionResult BrandTemplateDelete(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageMaintenance))
                return AccessDeniedView();

            var template = _brandTemplateService.GetBrandTemplateById(id);
            if (template == null)
                throw new ArgumentException("No template found with the specified id");

            _brandTemplateService.DeleteBrandTemplate(template);
