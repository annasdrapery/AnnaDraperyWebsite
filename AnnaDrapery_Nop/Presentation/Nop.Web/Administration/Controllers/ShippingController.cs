...
namespace Nop.Admin.Controllers
{
    public partial class ShippingController : BaseAdminController
	{
        ...
        #region Shipping Fee
        public ActionResult ShippingFeeByOrderTotal()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageShippingSettings))
                return AccessDeniedView();
        ...
        [HttpPost]
        public ActionResult ShippingFeeByOrderTotal(DataSourceRequest command)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageShippingSettings))
                return AccessDeniedView();

            var shippingFeeModel = _shippingService.GetAllShippingFees()
                .Select(x => x.ToModel())
                .ToList();
            var gridModel = new DataSourceResult
            {
                Data = shippingFeeModel,
                Total = shippingFeeModel.Count
            };

            return Json(gridModel);
        }

        public ActionResult CreateShippingFeeByOrderTotal()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageShippingSettings))
                return AccessDeniedView();

            var model = new ShippingFeeByOrderTotalModel();
            
            return View(model);
        }

        [HttpPost, ParameterBasedOnFormName("save-continue", "continueEditing")]
        public ActionResult CreateShippingFeeByOrderTotal(ShippingFeeByOrderTotalModel model, bool continueEditing)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageShippingSettings))
                return AccessDeniedView();
            ...
            {
                var sm = model.ToEntity();
                _shippingService.InsertShippingFee(sm);

                SuccessNotification(_localizationService.GetResource("Admin.Configuration.Shipping.ShippingFeeByOrderTotal.Added"));
                return continueEditing ? RedirectToAction("EditShippingFeeByOrderTotal", new { id = sm.Id }) : RedirectToAction("ShippingFeeByOrderTotal");
            }
        ...
        public ActionResult EditShippingFeeByOrderTotal(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageShippingSettings))
                return AccessDeniedView();

            var sm = _shippingService.GetShippingFeeById(id);
            if (sm == null)
                //No shipping found with the specified id
                return RedirectToAction("ShippingFeeByOrderTotal");
        ...
        [HttpPost, ParameterBasedOnFormName("save-continue", "continueEditing")]
        public ActionResult EditShippingFeeByOrderTotal(ShippingFeeByOrderTotalModel model, bool continueEditing)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageShippingSettings))
                return AccessDeniedView();

            var sm = _shippingService.GetShippingFeeById(model.Id);
            if (sm == null)
                //No shipping  found with the specified id
                return RedirectToAction("ShippingFeeByOrderTotal");
            ...
            {
                sm = model.ToEntity(sm);
                _shippingService.UpdateShippingFee(sm);
                SuccessNotification(_localizationService.GetResource("Admin.Configuration.Shipping.ShippingFeeByOrderTotal.Updated"));
                return continueEditing ? RedirectToAction("EditShippingFeeByOrderTotal", sm.Id) : RedirectToAction("ShippingFeeByOrderTotal");
            }
        ...
        [HttpPost]
        public ActionResult DeleteShippingFeeByOrderTotal(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageShippingSettings))
                return AccessDeniedView();
            ...
            _shippingService.DeleteShipmentFee(sm);

            SuccessNotification(_localizationService.GetResource("Admin.Configuration.Shipping.ShippingFeeByOrderTotal.Deleted"));
            return RedirectToAction("ShippingFeeByOrderTotal");
        }
        #endregion
    }
