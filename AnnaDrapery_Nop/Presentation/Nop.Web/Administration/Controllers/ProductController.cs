...
namespace Nop.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
            ...
            bool setPredefinedValues, bool excludeProperties)
        {
            ...
            {
                Text = _localizationService.GetResource("Admin.Catalog.Products.Fields.Brand.None"),
                Value = "0"
            });
            var brands = _brandService.GetAllBrands(showHidden: true).OrderBy(x => x.Name);
            ...
            // drapery attributes
            model.AvailableCollectionStyles = CollectionStyle.Wood.ToSelectList(false).ToList();
            model.AvailableCollectionStyles.Insert(0,new SelectListItem
            {
                Text = "--Select--",
                Value = "0"
            });
            model.AvailableFinialStyles = FinialStyle.Animal.ToSelectList(false).ToList();
            model.AvailableFinialStyles.Insert(0, new SelectListItem
            {
                Text = "--Select--",
                Value = "0"
            });
            model.AvailablePoleStyles = PoleStyle.Bamboo.ToSelectList(false).ToList();
            model.AvailablePoleStyles.Insert(0, new SelectListItem
            {
                Text = "--Select--",
                Value = "0"
            });
            model.AvailableBracketStyles = BracketStyle.Adjustable.ToSelectList(false).ToList();
            model.AvailableBracketStyles.Insert(0, new SelectListItem
            {
                Text = "--Select--",
                Value = "0"
            });
            model.AvailableAccessoriesStyles = AccessoriesStyle.AlligatorClips.ToSelectList(false).ToList();
            model.AvailableAccessoriesStyles.Insert(0, new SelectListItem
            {
                Text = "--Select--",
                Value = "0"
            });

            model.AvailablePoleDiameters = DraperyAttributeExtensions.AvailablePoleDiameter();
            model.AvailablePoleDiameters.Insert(0, new SelectListItem
            {
                Text = "--Select--",
                Value = "0"
            });
            model.AvailableOuterDiameters = DraperyAttributeExtensions.AvailableOuterDiameter();
            model.AvailableOuterDiameters.Insert(0, new SelectListItem
            {
                Text = "--Select--",
                Value = "0"
            });
            model.AvailableInnerDiameters = DraperyAttributeExtensions.AvailableInnerDiameter();
            model.AvailableInnerDiameters.Insert(0, new SelectListItem
            {
                Text = "--Select--",
                Value = "0"
            });
            model.AvailablePoleLengths = DraperyAttributeExtensions.AvailablePoleLength();
            model.AvailablePoleLengths.Insert(0, new SelectListItem
            {
                Text = "--Select--",
                Value = "0"
            });
            model.AvailableRodsOptions = RodsOption.BayOption.ToSelectList(false).ToList();
            model.AvailableRodsOptions.Insert(0, new SelectListItem
            {
                Text = "--Select--",
                Value = "0"
            });
            model.AvailableRodsTypes = RodsType.CafeRod.ToSelectList(false).ToList();
            model.AvailableRodsTypes.Insert(0, new SelectListItem
            {
                Text = "--Select--",
                Value = "0"
            });
            model.AvailableDecorativeTypes = DecorativeType.DecorativeCombo.ToSelectList(false).ToList();
            model.AvailableDecorativeTypes.Insert(0, new SelectListItem
            {
                Text = "--Select--",
                Value = "0"
            });
            model.AvailableAdjustableSizes = DraperyAttributeExtensions.AvailableAdjustableSizeList();
            model.AvailableAdjustableSizes.Insert(0, new SelectListItem
            {
                Text = "--Select--",
                Value = ""
            });
            ...
            //complete rod sets
            if (model.FinialProductId.HasValue)
            {
                var _finial = _productService.GetProductById(model.FinialProductId.Value);
                if (_finial!=null)
                {
                    model.FinialProductName = _finial.Name;
                }
            }
            if (model.PoleProductId.HasValue)
            {
                var _pole = _productService.GetProductById(model.PoleProductId.Value);
                if (_pole != null)
                {
                    model.PoleProductName = _pole.Name;
                }
            }
        }
        ...
        public ActionResult ProductList(DataSourceRequest command, ProductListModel model)
        {
            ...
            gridModel.Data = products.Select(x =>
            {
                ...
                //vendor
                var vendor = _vendorService.GetVendorById(x.VendorId);
                if (vendor != null)
                    productModel.VendorName = vendor.Name;
                //product template
                var productTemplate = _productTemplateService.GetProductTemplateById(x.ProductTemplateId);
                if (productTemplate != null)
                    productModel.ProductTemplateName = productTemplate.Name;
                return productModel;
            });
        ...
        public ActionResult Edit(int id)
        {
           ...
           // CalculatePriceCompleteRodSet("696", "287", "288", "1", "1");
            return View(model);
        }
        ...
        public ActionResult RequiredProductAddPopupList(DataSourceRequest command, ProductModel.AddRequiredProductModel model)
        {
            ...
            gridModel.Data = products.Select(x => 
                {
                    var _product = x.ToModel();
                    _product.Name = Server.HtmlEncode(_product.Name);
                    return _product;
                }
                );
            gridModel.Total = products.TotalCount;
        ...
        public ActionResult ComponentProductAddPopup(string productIdsInput,string selectedProductNameId, int productType = 0)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            model.SearchProductTypeId = productType;
            ViewBag.productIdsInput = productIdsInput;
            ViewBag.selectedProductNameId = selectedProductNameId;
        ...
        public ActionResult ProductAttributeMappingInsert(ProductModel.ProductAttributeMappingModel model)
        {
            ...
            foreach (var predefinedValue in predefinedValues)
            {
                var pav = new ProductAttributeValue
                {
                    ...
                    ExpectedProfit= predefinedValue.ExpectedProfit,
                    PictureId = predefinedValue.PictureId,
                    SupplierAttributeCode = predefinedValue.SupplierAttributeCode,
                    SupplierPartNumber = predefinedValue.SupplierPartNumber,
                    IsDisplayPriceWithFee = predefinedValue.IsDisplayPriceWithFee
                };
            ...
            var gridModel = new DataSourceResult
            {
                Data = values.Select(x =>
                {
                    ...
                    return new ProductModel.ProductAttributeValueModel
                    {
                        ...
                        Name = x.Name,//x.ProductAttributeMapping.AttributeControlType != AttributeControlType.ColorSquares ? x.Name : string.Format("{0} - {1}", x.Name, x.ColorSquaresRgb),
                        ...
                        SupplierAttributeCode = x.SupplierAttributeCode,
                        IsDisplayPriceWithFee = x.IsDisplayPriceWithFee
                    };
                }),
                Total = values.Count()
            };
        ...
        public ActionResult ProductAttributeValueCreatePopup(string btnId, string formId, ProductModel.ProductAttributeValueModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                var pav = new ProductAttributeValue
                {
                    ...
                    SupplierAttributeCode = model.SupplierAttributeCode,
                    SupplierPartNumber = model.SupplierPartNumber,
                    IsDisplayPriceWithFee = model.IsDisplayPriceWithFee,
                    FinialPartNumber = model.FinialPartNumber,
                    BracketPartNumber = model.BracketPartNumber,
                    PolePartNumber = model.PolePartNumber,
                    RingPartNumber = model.RingPartNumber,
        ...
        public ActionResult ProductAttributeValueEditPopup(int id)
        {
            ...
            var model = new ProductModel.ProductAttributeValueModel
            {
                ...
                ExpectedProfit = pav.ExpectedProfit,
                SupplierAttributeCode = pav.SupplierAttributeCode,
                SupplierPartNumber = pav.SupplierPartNumber,
                IsDisplayPriceWithFee = pav.IsDisplayPriceWithFee,
                FinialPartNumber = pav.FinialPartNumber,
                BracketPartNumber = pav.BracketPartNumber,
                PolePartNumber = pav.PolePartNumber,
                RingPartNumber = pav.RingPartNumber,
                ProductTypeId = (int)product.ProductType,
                Packaged = pav.Packaged,
                ComponentProductId = pav.ComponentProductId
            };
            if (model.ComponentProductId.HasValue)
            {
                var _component = _productService.GetProductById(model.ComponentProductId.Value);
                if (_component != null)
                {
                    model.ComponentProductName = _component.Name;
                }
            }
        ...
        public ActionResult ProductAttributeValueEditPopup(string btnId, string formId, ProductModel.ProductAttributeValueModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                pav.SupplierPartNumber = model.SupplierPartNumber;
                pav.SupplierAttributeCode = model.SupplierAttributeCode;
                pav.ExpectedProfit = model.ExpectedProfit;
                pav.IsDisplayPriceWithFee = model.IsDisplayPriceWithFee;
                //pav.FinialPartNumber = model.FinialPartNumber;
                //pav.BracketPartNumber = model.BracketPartNumber;
                //pav.PolePartNumber = model.PolePartNumber;
                //pav.RingPartNumber = model.RingPartNumber;
                pav.Packaged = model.Packaged;
                pav.ComponentProductId = model.ComponentProductId;
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult CalculatePriceCompleteRodSet(string currentProId, string finialId, string poleId, string numberOfFinials, string numberOfPoles)
        {
            //Complete Rod Set = 2 Finials + 1 Pole + 2 Brackets
            decimal price = 0;
            decimal finialQty = 2, poleQty = 1;
            decimal.TryParse(numberOfFinials, out finialQty);
            decimal.TryParse(numberOfPoles, out poleQty);
            var product = _productService.GetProductById(int.Parse("0" + currentProId));
            if (product != null)
            {
                var defaultColorName = string.Empty;
                var colorAttr = _productAttributeService.GetProductAttributeMappingsByProductId(product.Id).Where(x => x.ProductAttribute.Name.Trim().ToLower() == "color").FirstOrDefault();
                if (colorAttr != null)
                {
                ...
                var finial = _productService.GetProductById(int.Parse("0" + finialId));
                if (finialId != null)
                {
                    //get price follow to color
                    ...
                    {
                        var defaultFinialColor = finialColorAttr.ProductAttributeValues.Where(x => x.Name.Trim().ToLower() == defaultColorName.Trim().ToLower()).FirstOrDefault();
                        if (defaultFinialColor != null)
                        {
                            price += finialQty * (finial.Price + defaultFinialColor.PriceAdjustment);
                        }
                        else
                        {
                            price += finialQty * finial.Price;
                        }
                    }
                    else
                    {
                ...
                var pole = _productService.GetProductById(int.Parse("0" + poleId));
                if (poleId != null)
                {
                    //get price follow to color
                    ...
                    {
                        var defaultPoleColor = poleColorAttr.ProductAttributeValues.Where(x => x.Name.Trim().ToLower() == defaultColorName.Trim().ToLower()).FirstOrDefault();
                        if (defaultPoleColor != null)
                        {
                            price += poleQty * (pole.Price + defaultPoleColor.PriceAdjustment);
                        }
                        else
                        {
                            price += poleQty * pole.Price;
                        }
                    }
                    else
                    {
                ...
                if (bracketAttr != null)
                {
                    ...
                    {
                        var bracket = _productService.GetProductById(firstBracket.ComponentProductId.Value);
                        if (bracket != null)
                        {
                            //get price follow to color
                            var bracketColorAttr = _productAttributeService.GetProductAttributeMappingsByProductId(bracket.Id).Where(x => x.ProductAttribute.Name.Trim().ToLower() == "color").FirstOrDefault();
                            if (bracketColorAttr != null)
                            {
                                ...
                                {
                                    price += firstBracket.Quantity * (bracket.Price + defaultBracketColor.PriceAdjustment);
                                }
                                else
                                {
                                    price += firstBracket.Quantity * bracket.Price;
                                }
                            }
                            else
                            {
            ...
            return Json(price.ToString("0.00"));
        }
        #endregion
