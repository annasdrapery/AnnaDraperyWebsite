    ...
    public partial class ProductController : BasePublicController
    {
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var model = new ProductDetailsModel
            {
                ...
                MinRodWidth = product.MinRodWidth,
                MaxRodWidth = product.MaxRodWidth,
            ...
            if (product.BrandId > 0)
            {
                ...
                {                 
                    model.BrandModel = new VendorBriefInfoModel
                    {
                        Id = brand.Id,
                        Name = brand.GetLocalized(x => x.Name),
                        SeName = brand.GetSeName(),
                    };
                }
            }
            #endregion
            ...
            #region Rod Width
            var selectedRodWidth = updatecartitem != null ? Convert.ToInt32(updatecartitem.SelectedRodWidth) : product.MinRodWidth;
            double selectedRodWidthEven = updatecartitem != null ? (double)updatecartitem.SelectedRodWidtheven : 0;
            if (product.ProductType == ProductType.DecorativeTraverseRods || product.ProductType == ProductType.HeavyDutyCurtainRods)
            {
                
                if (Request.QueryString["selectedrodwidth"] != null)
                    int.TryParse(Request.QueryString["selectedrodwidth"], out selectedRodWidth);
               
                if (Request.QueryString["selectedrodwidtheven"] != null)
                    double.TryParse(Request.QueryString["selectedrodwidtheven"], out selectedRodWidthEven);
                for (int i = product.MinRodWidth; i <= product.MaxRodWidth; i++)
                {
                    model.AvailableRodWidths.Add(new SelectListItem
                    {
                        Text = i + " \"",
                        Value = i.ToString(),
                        Selected = (i == selectedRodWidth)
                    });
                }
                model.AvailableRodWidthEvens = CommonHelper.GetDimensionEvens(selectedValue:selectedRodWidthEven, maxValue: 1, seed: 0.25);
            }
            #endregion
            ...
            foreach (var attribute in productAttributeMapping)
            {
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        ...
                        {
                            decimal taxRate;
                            //get Color Name for Complete Rod Sets
                            string selectedColorName = null;
                            var colorAttr = productAttributeMapping.Where(x => x.ProductAttribute.Name.Trim().ToLower() == "color").FirstOrDefault();
                            if (colorAttr!= null)
                            {
                                var colorEntry = colorAttr.ProductAttributeValues.FirstOrDefault();
                                if (colorEntry != null)
                                {
                                    selectedColorName = colorEntry.Name;
                                }
                            }

                            decimal attributeValuePriceAdjustment = _priceCalculationService.GetProductAttributeValuePriceAdjustment(attributeValue, selectedColorName, (decimal)(selectedRodWidth + selectedRodWidthEven));
                            ...
                            if (attributeValue.IsDisplayPriceWithFee)
                            {
                                valueModel.PriceAdjustment = _priceFormatter.FormatPrice(model.ProductPrice.PriceValue + priceAdjustment, false, false);
                            }
                            else
                            {
                            ...
                            valueModel.IsDisplayPriceWithFee = attributeValue.IsDisplayPriceWithFee;
