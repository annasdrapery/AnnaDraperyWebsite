    ...
    public partial class CatalogController : BasePublicController
    {
            ...
            bool forceRedirectionAfterAddingToCart = false)
        {
            return this.PrepareProductOverviewModels(_workContext,
                ...
                forceRedirectionAfterAddingToCart, getAllPictureModel: true, brandService: _brandService, productAttributeService: _productAttributeService);
        }
        ...
        public ActionResult Category(int categoryId, CatalogPagingFilteringModel command)
        {
            ...
            //hardware
            PrepareHardwareProperties(model.PagingFilteringContext, command);
            ...
            var products = _productService.SearchProducts(out filterableSpecificationAttributeOptionIds,
                ...
                brandIds: (command.Br + "").TrimEnd(','),
                productTypeIds: (command.Cmp + "").TrimEnd(','),
                poleDiameters: (command.Pdm + "").TrimEnd(','),
                colllectionTypeIds: (command.Cls + "").TrimEnd(','),
                finialTypeIds: (command.Fs + "").TrimEnd(','),
                poleLengths: (command.Pl + "").TrimEnd(','),
                bracketTypeIds: (command.Bs + "").TrimEnd(','),
                accessoriesTypeIds: (command.As + "").TrimEnd(','),
                rodsTypeIds: (command.Rt + "").TrimEnd(','),
                decorativeTypeIds: (command.Dt + "").TrimEnd(','),
                rodsOptionTypeIds: (command.Rp + "").TrimEnd(','),
                poleTypeIds: (command.Ps + "").TrimEnd(','),
                outerDiameters: (command.Odm + "").TrimEnd(','),
                adjustableSizes: (command.Adjs + "").TrimEnd(','),
                storeId: _storeContext.CurrentStore.Id,
                visibleIndividuallyOnly: true,
        ...
        [ChildActionOnly]
        public ActionResult DraperyForYouNavigation()
        {
            var draperyForYouCategory = _categoryService.GetAllCategories("Drapery 4 You").FirstOrDefault();
            var draperyForYouChildren = new List<CategorySimpleModel>();
            if (draperyForYouCategory != null)
            {
                draperyForYouChildren = _categoryService.GetAllCategoriesByParentCategoryId(draperyForYouCategory.Id)
                    .Select(x => new CategorySimpleModel()
                    {
                        Id = x.Id,
                        Name = x.Name,
                        SeName = x.GetSeName(),
            ...
            var model = new CategoryNavigationModel
            {
                Categories = draperyForYouChildren
            };
            return PartialView(model);
        }
        [ChildActionOnly]
        public ActionResult DraperyForYouFooter()
        {
            var draperyForYouCategory = _categoryService.GetAllCategories("Drapery 4 You").FirstOrDefault();
            var draperyForYouChildren = new List<CategorySimpleModel>();
            if (draperyForYouCategory != null)
        ...
        [ChildActionOnly]
        public ActionResult DraperyCollectionFooter()
        {
            var draperyForYouChildren = new List<CategorySimpleModel>();

            draperyForYouChildren = _categoryService.GetAllCategoriesByParentCategoryId(0)
                .Select(x => new CategorySimpleModel()
                {
                    Id = x.Id,
                    Name = x.Name,
        ...
        public ActionResult TopMenu()
        {
            ...
            var model = new TopMenuModel
            {
                ...
                DesignerLoginEnabled = _permissionService.Authorize(StandardPermissionProvider.AccessPagesUnderDesignerLogin)
            };
            //brands
            var brands = this._brandService.GetAllBrands();
            foreach (var brand in brands)
            {
                var modelBrand = brand.ToModel();

                //prepare picture model
                int pictureSize = _mediaSettings.ManufacturerThumbPictureSize;
                var brandPictureCacheKey = string.Format(ModelCacheEventConsumer.BRAND_PICTURE_MODEL_KEY, brand.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
                modelBrand.PictureModel = _cacheManager.Get(brandPictureCacheKey, () =>
                {
                    ...
                    var pictureModel = new PictureModel
                    {
                        FullSizeImageUrl = _pictureService.GetPictureUrl(picture),
                        ImageUrl = _pictureService.GetPictureUrl(picture, pictureSize),
                        Title = string.Format(_localizationService.GetResource("Media.Manufacturer.ImageLinkTitleFormat"), modelBrand.Name),
                        AlternateText = string.Format(_localizationService.GetResource("Media.Manufacturer.ImageAlternateTextFormat"), modelBrand.Name)
                    };
                    return pictureModel;
                });
                model.Brands.Add(modelBrand);
            }
            return PartialView(model);
        }
        ...
        #region Brand
        [NopHttpsRequirement(SslRequirement.No)]
        public ActionResult Brand(int brandId, CatalogPagingFilteringModel command)
        {
            ...
            //Check whether the current user has a "Manage catalog" permission
            //It allows him to preview a brand before publishing
            if (!brand.Published && !_permissionService.Authorize(StandardPermissionProvider.ManageBrands))
                return InvokeHttp404();
            ...
            //page size
            PreparePageSizeOptions(model.PagingFilteringContext, command,
                brand.AllowCustomersToSelectPageSize,
                brand.PageSizeOptions,
                brand.PageSize);
            ...
            //template
            var templateCacheKey = string.Format(ModelCacheEventConsumer.BRAND_TEMPLATE_MODEL_KEY, brand.BrandTemplateId);
            var templateViewPath = _cacheManager.Get(templateCacheKey, () =>
            {
                var template = _brandTemplateService.GetBrandTemplateById(brand.BrandTemplateId);
                if (template == null)
                    template = _brandTemplateService.GetAllBrandTemplates().FirstOrDefault();
                if (template == null)
                    throw new Exception("No default template could be loaded");
                return template.ViewPath;
            });
            ...
            //activity log
            _customerActivityService.InsertActivity("PublicStore.ViewBrand", _localizationService.GetResource("ActivityLog.PublicStore.ViewBrand"), brand.Name);

            return View(templateViewPath, model);
        }

        [NonAction]
        protected virtual void PrepareHardwareProperties(CatalogPagingFilteringModel pagingFilteringModel, CatalogPagingFilteringModel command)
        {
            if (pagingFilteringModel == null)
                throw new ArgumentNullException("pagingFilteringModel");
            ...
            if (command == null)
                throw new ArgumentNullException("command");
            //components
            var components = ProductType.SimpleProduct.ToSelectList(false).Where(x => int.Parse("0" + x.Value) > 10).ToList();
            if (!String.IsNullOrEmpty(command.Cmp))
	        {
		         var arrSelectedCompId = command.Cmp.Split(new []{","},StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in components)
	            {
                    
		             if (arrSelectedCompId.Any(x=>x== item.Value))
	                {
		                 item.Selected = true;
	                }
	            }
	        }
            pagingFilteringModel.AvailableComponents = components;
            pagingFilteringModel.Cmp = command.Cmp;
            //poles diameter
            var polediameters = DraperyAttributeExtensions.AvailablePoleDiameter().ToList();
            if (!String.IsNullOrEmpty(command.Pdm))
	        {
		         var arrSelectedPoleDiameter = command.Pdm.Split(new []{","},StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in polediameters)
	            {
                    double _selectedValue;
		             if ( arrSelectedPoleDiameter.Any(x => double.TryParse(x,out _selectedValue) && double.Parse(x) == double.Parse(item.Value)))
	                {
		                 item.Selected = true;
	                }
	            }
	        }
            pagingFilteringModel.AvailablePoleDiameters = polediameters;
            pagingFilteringModel.Pdm = command.Pdm;
            //collection style
            var collectionStyles = CollectionStyle.Wood.ToSelectList(false).ToList();
            if (!String.IsNullOrEmpty(command.Cls))
            {
                var arrSelectedCollectionStyle = command.Cls.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in collectionStyles)
                {
                    if (arrSelectedCollectionStyle.Any(x => x == item.Value))
                    {
                        item.Selected = true;
                    }
                }
            }
            pagingFilteringModel.AvailableCollectionStyles = collectionStyles;
            pagingFilteringModel.Cls = command.Cls;
            //brand
            var brands = _brandService.GetAllBrands().Select(x => new SelectListItem() { Value = x.Id.ToString(), Text = x.Name }).ToList();
            if (!String.IsNullOrEmpty(command.Br))
            {
                var arrSelectedBrand = command.Br.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in brands)
                {
                    if (arrSelectedBrand.Any(x => x == item.Value))
                    {
                        item.Selected = true;
                    }
                }
            }
            pagingFilteringModel.AvailableBrands = brands;
            pagingFilteringModel.Br = command.Br;
            //finial style
            var finialStyles = FinialStyle.Animal.ToSelectList(false).ToList();
            if (!String.IsNullOrEmpty(command.Fs))
            {
                var arrFinialStyle = command.Fs.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in finialStyles)
                {
                    if (arrFinialStyle.Any(x => x == item.Value))
                    {
                        item.Selected = true;
                    }
                }
            }
            pagingFilteringModel.AvailableFinialStyles = finialStyles;
            pagingFilteringModel.Fs = command.Fs;
            //pole style
            var poleStyles = PoleStyle.Bamboo.ToSelectList(false).ToList();
            if (!String.IsNullOrEmpty(command.Ps))
            {
                var arrPoleStyle = command.Ps.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in poleStyles)
                {
                    if (arrPoleStyle.Any(x => x == item.Value))
                    {
                        item.Selected = true;
                    }
                }
            }
            pagingFilteringModel.AvailablePoleStyles = poleStyles;
            pagingFilteringModel.Ps = command.Ps;
            //pole length
            var poleLengths = DraperyAttributeExtensions.AvailablePoleLength().ToList();
            if (!String.IsNullOrEmpty(command.Pl))
            {
                var arrPoleLength = command.Pl.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in poleLengths)
                {
                    if (arrPoleLength.Any(x => x == item.Value))
                    {
                        item.Selected = true;
                    }
                }
            }
            pagingFilteringModel.AvailablePoleLengths = poleLengths;
            pagingFilteringModel.Pl = command.Pl;
            //Bracket Style
            var bracketStyles = BracketStyle.Adjustable.ToSelectList(false).ToList();
            if (!String.IsNullOrEmpty(command.Bs))
            {
                var arrBracketStyles = command.Bs.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in bracketStyles)
                {
                    if (arrBracketStyles.Any(x => x == item.Value))
                    {
                        item.Selected = true;
                    }
                }
            }
            pagingFilteringModel.AvailableBracketStyles = bracketStyles;
            pagingFilteringModel.Bs = command.Bs;
            //Inner Diameter
            var innerDiameter = DraperyAttributeExtensions.AvailableInnerDiameter().ToList();
            if (!String.IsNullOrEmpty(command.Idm))
            {
                var arrInnerDiameter = command.Idm.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in innerDiameter)
                {
                    double _selectedValue;
                    if (arrInnerDiameter.Any(x => double.TryParse(x, out _selectedValue) && double.Parse(x) == double.Parse(item.Value)))
                    {
                        item.Selected = true;
                    }
                }
            }
            pagingFilteringModel.AvailableInnerDiameters = innerDiameter;
            pagingFilteringModel.Idm = command.Idm;
            //Outer Diameter
            var outerDiameter = DraperyAttributeExtensions.AvailableOuterDiameter().ToList();
            if (!String.IsNullOrEmpty(command.Odm))
            {
                var arrOuterDiameter = command.Odm.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in outerDiameter)
                {
                    double _selectedValue;
                    if (arrOuterDiameter.Any(x => double.TryParse(x, out _selectedValue) && double.Parse(x) == double.Parse(item.Value)))
                    {
                        item.Selected = true;
                    }
                }
            }
            pagingFilteringModel.AvailableOuterDiameters = outerDiameter;
            pagingFilteringModel.Odm = command.Odm;
            //Accessories Style
            var accessoriesStyles = AccessoriesStyle.AlligatorClips.ToSelectList(false).ToList();
            if (!String.IsNullOrEmpty(command.As))
            {
                var arrAccessoriesStyle = command.As.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in accessoriesStyles)
                {
                    if (arrAccessoriesStyle.Any(x => x == item.Value))
                    {
                        item.Selected = true;
                    }
                }
            }
            pagingFilteringModel.AvailableAccessoriesStyles = accessoriesStyles;
            pagingFilteringModel.As = command.As;
            //Rods Type
            var rodsTypes = RodsType.CafeRod.ToSelectList(false).ToList();
            if (!String.IsNullOrEmpty(command.Rt))
            {
                var arrRodsType = command.Rt.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in rodsTypes)
                {
                    if (arrRodsType.Any(x => x == item.Value))
                    {
                        item.Selected = true;
                    }
                }
            }
            pagingFilteringModel.AvailableRodsTypes = rodsTypes;
            pagingFilteringModel.Rt = command.Rt;
            //Decorative Type
            var decorativeTypes = DecorativeType.DecorativeCombo.ToSelectList(false).ToList();
            if (!String.IsNullOrEmpty(command.Dt))
            {
                var arrDecorativeType = command.Dt.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in decorativeTypes)
                {
                    if (arrDecorativeType.Any(x => x == item.Value))
                    {
                        item.Selected = true;
                    }
                }
            }
            pagingFilteringModel.AvailableDecorativeTypes = decorativeTypes;
            pagingFilteringModel.Dt = command.Dt;
            //Rods Option
            var rodsOptions = RodsOption.BayOption.ToSelectList(false).ToList();
            if (!String.IsNullOrEmpty(command.Rp))
            {
                var arrRodsOptions = command.Rp.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in rodsOptions)
                {
                    if (arrRodsOptions.Any(x => x == item.Value))
                    {
                        item.Selected = true;
                    }
                }
            }
            pagingFilteringModel.AvailableRodsOptions = rodsOptions;
            pagingFilteringModel.Rp = command.Rp;
            //Adjustable Sizes
            var adjustableSizes = DraperyAttributeExtensions.AvailableAdjustableSizeList();
            if (!String.IsNullOrEmpty(command.Adjs))
            {
                var arrAdjustableSizes = command.Adjs.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in adjustableSizes)
                {
                    if (arrAdjustableSizes.Any(x => x == item.Value))
                    {
                        item.Selected = true;
                    }
                }
            }
            pagingFilteringModel.AvailableAdjustableSizes = adjustableSizes;
            pagingFilteringModel.Adjs = command.Adjs;
        }
        #endregion
